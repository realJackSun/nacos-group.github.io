...
If you choose to use official images, please clone the 'nacos-docker' project.
...
Then, inject the environment parameter 

```powershell
NACOS_AUTH_ENABLE=true
```

by modify the docker-compose files. For example, if you choose to use example/standalone-derby.yaml, just add environment parameters into the 'environment' part, just like: 
...
Then run the docker-compose commands like
```powershell
